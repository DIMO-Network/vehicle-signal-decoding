...
type DeviceTemplateService interface {
	...
	ResolveDeviceConfiguration(ctx context.Context, ud *pb.UserDevice, vehicle *gateways.VehicleInfo, ethAddress common2.Address) (*appmodels.DeviceConfigResponse, error)
}
...
func (dts *deviceTemplateService) ResolveDeviceConfiguration(ctx context.Context, ud *pb.UserDevice, vehicle *gateways.VehicleInfo, ethAddress common2.Address) (*appmodels.DeviceConfigResponse, error) {
	...
	matchedTemplate, err := dts.selectAndFetchTemplate(ctx, canProtocl, powertrain, ud.DeviceDefinitionId, vehicle)
	if err != nil {
		return nil, err
	}
