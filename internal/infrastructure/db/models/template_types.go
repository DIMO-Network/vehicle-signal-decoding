...
func (o *TemplateType) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(templateTypeAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(templateTypePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert template_types, could not build conflict column list")
			}

			conflict = make([]string, len(templateTypePrimaryKeyColumns))
			copy(conflict, templateTypePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"vehicle_signal_decoding_api\".\"template_types\"", updateOnConflict, ret, update, conflict, insert, opts...)
