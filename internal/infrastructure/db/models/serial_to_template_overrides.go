...
func (o *SerialToTemplateOverride) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(serialToTemplateOverrideAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(serialToTemplateOverridePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert serial_to_template_overrides, could not build conflict column list")
			}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"vehicle_signal_decoding_api\".\"serial_to_template_overrides\"", updateOnConflict, ret, update, conflict, insert, opts...)
